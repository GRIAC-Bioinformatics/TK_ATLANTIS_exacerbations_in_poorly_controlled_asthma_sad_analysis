# This code will use multivariate cox regression to predict time to exacerbation 
# in asthma as a whole groups 
# - using 3 parameters to define severe SAD
# - correcting for asthma comtrol 
...
# add exacerbation info to the main table 
  left_join(db, by = c("PT" = "PT"))


## add severe SAD parameters: 
...
db_exacerbations <- db_exacerbations %>% 
  mutate(
    ...
    poor_asthma_control = if_else(ACQ6group == "poorly controlled asthma", TRUE, FALSE),
...
sad_survival_multi <- function(sad_variable, db_exacerbations) {
  # prepare df
  ...
  # Define the Formula - correct for sad and asthma control group
  formula_obj <- as.formula(
    paste0("Surv(time, exacerbation) ~ AGE + SEX + ex_smoker + GINA45 + more_than_1_exac_last_year + LABEOSV + B_FEV1PNVG + poor_asthma_control + ", 
           sad_variable)
  )
  # run cox regression
  ...
  readr::write_delim(model_table,
                              file.path(cfg$paths$output_tables, 
                              paste0("wholegroup_", sad_variable,
                             "_full_multivar_cox.csv")), delim = ";")
}


#########################################################
# Define sad variables (colnames)
