# This code will use univariate Negative Binomial Model to predict exacerbation frequencies in 3 asthma control groups 
...
#########################################################
# function to perform univariate negative binomial with Offset
#########################################################

sad_nb_uni <- function(sad_variable, db_exacerbations, subgroup = NULL) {
...
for (sad_definition in sad_variables) {
  for (control_subgroup in asthma_control) {
    results_list[[paste(sad_definition, control_subgroup, sep = "_")]] <- 
      sad_nb_uni(sad_definition, db_exacerbations, control_subgroup)
  }
  }
...
write.xlsx(results_table,
                   file.path(cfg$paths$output_tables, "sad_uni_model", 
                             "sad_uni_NB_model_all_sad_all_asthma.xlsx"),
                   delim = ";")
